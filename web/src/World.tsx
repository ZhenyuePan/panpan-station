...
function Hotspot({ p, number, label, color, onClick }: { p: V3; number?: string; label: string; color?: string; onClick: () => void }) {
  return <Html position={p} center zIndexRange={[8, 1]}><button className="world-label chat-hotspot" onClick={onClick} style={{ '--hotspot': color || '#ffd1a1' } as React.CSSProperties}>{number && <span>{number}</span>}{label}</button></Html>;
}
  ...
  return <>
    ...
    <group position={[0, -.4, 0]}>
      ...
      <group position={[-2.8, 0, -1.94]} onClick={e => { e.stopPropagation(); navigate('blog'); }} onPointerOver={() => { document.body.style.cursor = 'pointer'; }} onPointerOut={() => { document.body.style.cursor = ''; }}>
        ...
        <Beacon p={[.56, 1.74, .68]} reduced={reduced} />
      </group>
      ...
      <group position={[.15, 0, -1.48]} onClick={e => { e.stopPropagation(); navigate('projects'); }} onPointerOver={() => { document.body.style.cursor = 'pointer'; }} onPointerOut={() => { document.body.style.cursor = ''; }}>
        ...
        <Beacon p={[1.31, 1.61, .4]} color="#bce9cd" reduced={reduced} />
      </group>
      {/* Chair. */}
      ...
      <group position={[2.95, 2.12, -2.94]} onClick={e => { e.stopPropagation(); navigate('forum'); }} onPointerOver={() => { document.body.style.cursor = 'pointer'; }} onPointerOut={() => { document.body.style.cursor = ''; }}>
        ...
        <Beacon p={[.5, .35, .18]} color="#ffcf8b" reduced={reduced} />
      </group>
      ...
      {[0, 1, 2].map(i => <group key={i}><Box p={[3.22, 1.07 - i * .29, -.465]} s={[.64, .17, .022]} color="#252836" /><Box p={[3.42, 1.07 - i * .29, -.448]} s={[.05, .04, .012]} color={C.cyan} glow={2} /></group>)}<Beacon p={[3.43, 1.1, -.43]} color="#aee6ff" reduced={reduced} /></group>
      ...
      <Hotspot p={[2.6, 2.25, 1.72]} label="和潘潘聊聊" color="#bce9cd" onClick={chat} />
    </group>
